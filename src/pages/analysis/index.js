...
const $analysisPageTemplate = $('#am-template-page-analysis');
...
const AnalysisPage = Backbone.View.extend({
  tagName: 'div',
  ...
  render() {
    ...
    const roles = store.user.get('roles');
    let tabItems = UserTabs;
  ...
  renderPeoChart() {
    const dom = this.$el.find('.analysis-peo-chart')[0];
  ...
  renderHomelessPeoChart() {
    const dom = this.$el.find('.analysis-homeless-peo-chart')[0];
  ...
  renderAgeChart() {
    const dom = this.$el.find('.analysis-age-chart')[0];
  ...
  renderPassChart() {
    const dom = this.$el.find('.analysis-pass-time-chart')[0];
    this._mypasstimeChart = echarts.init(dom);
    const dataMap = {};
    function dataFormatter(obj) {
      const pList = ['00:00','01:00','02:00','03:00','04:00','05:00','06:00','07:00','08:00','09:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00','21:00','22:00','23:00'];
      let temp = 0;
  ...
  onPageResize() {
    // const analysischartdivWidth = this.$el.find('.analysis-chart')[0].clientWidth * 0.48;
    // const analysischartdivHeight = analysischartWidth * 1.5;
    // const analysischartWidth = analysischartdivWidth;
    // const analysischartHeight = analysischartdivHeight;
    const controldivheight = this.$el.find('.analysis-chart')[0].clientWidth * 0.6;
    const divWidth = this.$el.find('.control-chart')[0].clientWidth;
    const divHeight = divWidth * 1.2;
    const chartWidth = divWidth;
    const chartHeight = divHeight;
    const changePie = chartHeight / this._oldHeight;


    if ( chartWidth === this._oldWidth && chartHeight === this._oldHeight ) {
      return;
    }
    // const $analysisChart = this.$el.find('.am-all-chart-position');
    // $analysisChart.css('width', `${analysischartWidth}px`);
    // $analysisChart.css('height', `${analysischartHeight}px`);
    const $controlchart = this.$el.find('.control-chart');
    $controlchart.css('height', `${controldivheight}px`);

    const $peochart = this.$el.find('.analysis-peo-chart');
    $peochart.css('width', `${chartWidth}px`);
    $peochart.css('height', `${chartHeight}px`);
    const $homelesspeochart = this.$el.find('.analysis-homeless-peo-chart');
    $homelesspeochart.css('width', `${chartWidth}px`);
    $homelesspeochart.css('height', `${chartHeight}px`);
    const $agechart = this.$el.find('.analysis-age-chart');
    $agechart.css('width', `${chartWidth}px`);
    $agechart.css('height', `${chartHeight}px`);
    const $passtimechart = this.$el.find('.analysis-pass-time-chart');
